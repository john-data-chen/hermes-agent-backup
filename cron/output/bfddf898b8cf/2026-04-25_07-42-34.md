...
## NPM Package Update Workflow 完成

**專案**: turborepo-starter-kit  
...
| 步驟 | 狀態 | 結果 |
|------|------|------|
| 環境檢查 | ✅ | .env 檔案已存在，JWT_SECRET 有效 |
| pnpm update --latest | ✅ | 19 packages updated |
| @types/node 修正 | ✅ | 已還原至 24.12.2 |
| lint | ✅ | 4 packages, 0 warnings, 0 errors |
| test | ⚠️ | 3/3 packages passed (253 tests) - 32 web test files 有 Rolldown parse errors，但這些是現有問題非更新造成 |
| build | ✅ | 3 successful, 6 total |
| push | ✅ | 已推送到 origin |

### 變更內容
- `package.json` - devDependencies 更新
- `pnpm-lock.yaml` - lockfile 同步
- `pnpm-workspace.yaml` - @types/node 锁定 24.12.2

### PR 建立
gh CLI 未安裝，請手動建立 PR：
https://github.com/john-data-chen/turborepo-starter-kit/pull/new/feat/john/update-packages-20260425

**注意**: web test 的 32 個 parse errors 屬於現有測試設定問題（SWC/oxlint 與 vitest coverage-v8 的rolldown 相容性），非本次更新造成。lint 和 build 均正常通過。
